...
from keyboards.keyboard_utils import write_show, more_end, more_end_repet
...
@router.message(Text(text=[LEXICON_MUSCLE['back'], LEXICON['more_approach']]), StateFilter(NewOrder.start_training))
...
async def write_approaches(message: Message, state: FSMContext):
    ...
    await message.answer('Подход записан. Сделаете еще?', reply_markup=more_end_repet)
    await state.clear()

# Хэндлер обрабатывает согласие на новый подход. Переносит в другое состояние
@router.message(Text(text=LEXICON['more_approach']))

...
@router.message(Text(text=[LEXICON_ACTIVITY['workout'], LEXICON['more_approach_workout']]),
                StateFilter(Activity.activity_change, Approaches.more_repet_workout))
...
@router.message(Text(text=LEXICON_MUSCLE['back']), StateFilter(Change_muscle.change_muscle_workout))
async def back_repetitions(message: Message, state: FSMContext):
    await message.answer('Выберите упражнение', reply_markup=back_kb_workout)
...
@router.message(Text(text=[LEXICON_REPETITIONS_BACK['pull_ups'], LEXICON_REPETITIONS_BACK['wide_grip_pull_ups'],
                           LEXICON_REPETITIONS_BACK['parallel_grip_pull_ups'], LEXICON_REPETITIONS_BACK['australian_pull_ups'],
                           LEXICON['more_approach_workout']]), StateFilter(MuscleGroup.back))
async def write_repetition(message: Message, state: FSMContext):
    ...
    await state.set_state(Approaches.start_repet_workout)
...
async def warning_not_change(message: Message):
    await message.answer('Пожалуйста, воспользуйтесь кнопками!\n\n'
                         ...
                         'отправьте команду /cancel', reply_markup=back_kb_workout)

@router.message(Text(text=LEXICON_MUSCLE['chest']), Change_muscle.change_muscle_workout)
async def chest_repetitions(message: Message, state: FSMContext):
    await message.answer('Выберите упражнение', reply_markup=chest_kb_workout)
...
@router.message(Text(text=[LEXICON_REPETITIONS_CHEST['push_ups_on'], LEXICON_REPETITIONS_CHEST['push_ups'],
                           ...
                           LEXICON['more_approach_workout']]), StateFilter(MuscleGroup.chest))
async def write_repetition(message: Message, state: FSMContext):
    await state.update_data(user_id=message.from_user.id)
    await state.update_data(date_train=datetime.now().date())
    await state.update_data(name=message.text)
...
@router.message(Text(text=[LEXICON_REPETITIONS_SHOULDERS['bench_press_standing'], LEXICON_REPETITIONS_SHOULDERS['bench_press_sitting'],
                           LEXICON['more_approach_workout']]), StateFilter(MuscleGroup.shoulders))
async def write_repetition(message: Message, state: FSMContext):
    await state.update_data(user_id=message.from_user.id)
    await state.update_data(date_train=datetime.now().date())
    await state.update_data(name=message.text)
...
@router.message(Text(text=[LEXICON_REPETITIONS_BICEPS['reverse_grip_pull_ups'], LEXICON_REPETITIONS_BICEPS['curling_arms_for_biceps'],
                           LEXICON['more_approach_workout']]), StateFilter(MuscleGroup.biceps))
async def write_repetition(message: Message, state: FSMContext):
    await state.update_data(user_id=message.from_user.id)
    await state.update_data(date_train=datetime.now().date())
    await state.update_data(name=message.text)
...
@router.message(Text(text=[LEXICON_REPETITIONS_TRICEPS['push_ups_between_benches'], LEXICON['more_approach_workout']]), StateFilter(MuscleGroup.triceps))
async def write_repetition(message: Message, state: FSMContext):
    await state.update_data(user_id=message.from_user.id)
    await state.update_data(date_train=datetime.now().date())
    await state.update_data(name=message.text)
...
@router.message(Text(text=[LEXICON_REPETITIONS_PRESS['torso_lift'],
                           ...
                           LEXICON['more_approach_workout']]), StateFilter(MuscleGroup.press))
async def write_repetition(message: Message, state: FSMContext):
    await state.update_data(user_id=message.from_user.id)
    await state.update_data(date_train=datetime.now().date())
    await state.update_data(name=message.text)
...
@router.message(Text(text=[LEXICON_REPETITIONS_THIGH['sit_ups'], LEXICON_REPETITIONS_THIGH['jumping'],
                           LEXICON['more_approach_workout']]), StateFilter(MuscleGroup.thigh))
...
@router.message(Text(text=[LEXICON_REPETITIONS_lOWER_LEG['rise_on_toes'],
                           LEXICON['more_approach_workout']]), StateFilter(MuscleGroup.lower_leg))
async def write_repetitions(message: Message, state: FSMContext):
    await state.update_data(user_id=message.from_user.id)
    await state.update_data(date_train=datetime.now().date())
    await state.update_data(name=message.text)
...
@router.message(StateFilter(Approaches.start_repet_workout))
async def write_approaches(message: Message, state: FSMContext, request: Request):
    ...
    if repetit.isdigit():
        ...
        await message.answer('Упражнение записано. Продолжаем11?', reply_markup=more_end_repet_workout)
        await state.set_state(Approaches.more_repet_workout)
    else:
